...
relevant.specimens = 'dali' #specific specimen name OR list of specimens e.g., All_specimens
...
#calculate scores (after chatGPT)
...
for (specimen in relevant.specimens) {
  ...
  #####Plot each specimen individually
  if(plot.specimens == TRUE){
    
    mList = output[[2]]
    mList = mList[c('NPH.vis','FhTYDiff.neu.nm','ZYB.vis','SkFltnsCS.neu.nm',
      ...
      compact() #remove NULL elements from list
    pList = list()
    
    for(mName in names(mList)){
      m = mList[[mName]]
      fig.name = pred[pred$long.name == mName,'short.name']
      img = paste('illustrator/Test Subjects SVGs/1x/',specimen,'.png', sep = '')
      ...
      m = m %>%
        filter(group %in% c(specimen,'AMH','NE'))
      
      m[m$group %in% c('NE'),'group'] = 'N'
      
      if(mName %in% c('PRA.NAA.Prin1.vis.nm','glab.prot.neu.nm')){
        m$value = -m$value  #to account for the correlation correction
      }
      ...
      p = ggplot(data = m,aes(x = group,y=value,))+
        ...
        geom_boxplot(data = subset(m, group %in% c('AMH','N')),
                     aes(fill=group),outlier.color = NA,
                     position=position_dodge(0), colour = 'black',
                     coef = 1.5)+ # To remove the boxplot whiskers
        scale_fill_manual(values = c('#3B94D1', '#FBB040'))+ #To select the right colors for the boxplots
        #geom_jitter(data = subset(m, group == specimen),aes(y=value),width = 0,size = 5)+
        geom_image(data = subset(m, group == specimen), image = img, size = 0.7)+
        xlab(fig.name)+
        coord_flip()+
        theme(panel.background = element_blank(),
              ...
              axis.text.y = element_blank(), #element_text(angle = 90),
    ...
    ggsave(filename = paste(specimen,'_Fig1.svg',sep =''),
           width = 3,height = 10, path = output.dir, bg = 'white')
    
  }

  if(plot.percentiles == TRUE){
    percentiles = output[[3]]
    percentiles$d = percentiles$d*2 #to change the tange into -1<x<1
    percentiles$Vs = factor(percentiles$Vs,levels=c("NE","AMH"))
    
    percentiles$pred_vs = paste(percentiles$related.prediction,percentiles$Vs,sep = "_")
      percentiles <- percentiles %>%
      arrange(related.prediction)%>%
      arrange(d)

    percentiles$pred_vs <- factor(percentiles$pred_vs, levels = percentiles$pred_vs)

    percentiles$related.prediction.clean <- percentiles$related.prediction %>%
      str_to_lower() %>% 
      str_replace_all("_", " ")
    
    percentiles <- percentiles %>%
      mutate(text.pos = if_else(d < 0, 0, d))
    
    ggplot(data = percentiles, aes(x = d, y = pred_vs, fill = Vs)) +
      geom_bar(stat = 'identity', width = 0.75,color = 'black', size = 0.5) +
      geom_vline(xintercept = 0, color = "black", size = 1) +
      geom_text(aes(x = text.pos+0.02), label = percentiles$related.prediction.clean,hjust = "left")+
      theme_minimal() +
      ...
      scale_fill_manual(values=c('#FBB040','#3B94D1'),
                        labels = c('NE' = 'Neanderthals',
                               'AMH' = 'AMHs'))
      
    #ggsave(filename = paste(specimen,'_percentile.svg',sep =''),
    #  width = 10,height = 6, path = 'final graphs after revision/percentile_plots', bg = 'white')     
...
if (permutate == TRUE) {
  ...
  # Calculate permutation success metrics
  per.binom <- as.numeric(permutations$binom.score)
  pval$perm_success_binom <- sum(pval$binom.score > per.binom)

  per.matched.d <- as.numeric(permutations$match_d_over_0.5)
  pval$perm_success_matched_d <- sum(pval$match_d_over_0.5 > per.matched.d)

  per.U <- as.numeric(permutations$wilcox.score)
  pval$perm_success_U <- sum(pval$wilcox.score > per.U)

  per.mean.d <- as.numeric(permutations$mean.d)
  pval$perm_success_mean_d <- sum(pval$mean.d > per.mean.d)

  per.comb_score <- as.numeric(permutations$comb_score)
  pval$perm_success_dist <- sum(pval$comb_score > per.comb_score)
...
#Add binom pval and wilcox pval
...
#Add adjustment to permutations test


#pvals$perm.p.adjusted = p.adjust(pvals$perm.p.val,method = 'BH')

write.csv(pvals,'final_results_without_normalization_permuted.csv',row.names = FALSE)
getwd()
#rm(list = ls())
...
#temp - fix pvalues to only include MPH
```{r}
temp_pvals <- read.csv("results/permutations/final_supplementary_results_harbin_t.csv")

#Add binom pval and wilcox pval

test.subjects = temp_pvals %>%
  filter((group %in% c('MPH','LMPEA'))&total_predictions>4) %>%
  mutate(temp_pvals = p.adjust(perm.p.val,method = 'BH'))

others = temp_pvals%>%
  filter(!group %in% c('MPH','LMPEA')|
           (group %in% c('MPH','LMPEA')&total_predictions<5))%>%
  mutate(perm.p.val = NA)



#Add wilcox pval

temp_pvals = bind_rows(test.subjects,others)
temp_pvals = temp_pvals[order(temp_pvals$perm.p.val),]
temp_pvals = temp_pvals[order(temp_pvals$match_rate,decreasing = TRUE),]

write.csv(temp_pvals,"final graphs after revision/main_results_table/final_supplementary_result_not_formatted.csv",row.names = FALSE)

```



# Supplementary statistical analysis for glenoid fossa
```{r}
GF.mat = mat
...
#         width = 10,height = 10, bg = 'white')
  ...
  #Create PCA 
     ...
     autoplot(pc,x = xAxis, y = yAxis, data = filtered , fill = 'group',
              colour = 'group',label = TRUE, shape = TRUE, frame = TRUE,loadings = FALSE)+
     theme(panel.background = element_blank(),
           legend.position='none')+
     scale_color_manual(values = c('MPH' = 'red', 'NE' = '#FBB040', 'AMH' = '#3B94D1', 'ERC' = 'lawngreen'))+
      scale_fill_manual(values = c('MPH' = 'red', 'NE' = '#FBB040', 'AMH' = '#3B94D1', 'ERC' = 'lawngreen'))+
       ggtitle(paste(names(types)[i],'maxSpecNA',maxSpecNA,'maxMeasurementNA ',maxMeasurementNA))
#  ggsave(filename = paste(PCAdir,name,'PCA.svg',sep =''),
#         width = 10,height = 10, bg = 'white')
#  write.csv(filtered,paste(PCAdir,name,'.csv',sep =''),row.names = FALSE)
  
  
  temp = ggplot(test,aes(x = PC1,y = PC2,fill = Group,colour = Group))+
...
#  ggsave(filename = paste(PCAdir,'V2',name,'PCA.svg',sep =''),device = 'svg',
#         width = 10,height = 10, bg = 'white')  
...
for(var in c('minusLogPvalU','minusLogPvalBin','match_rate','finalScore' )){#,'minusLogPermU','minusLogPermBin')){
  if(var == 'minusLogPvalU'){
    name = 'U Test Fit Score'
  }else if(var == 'minusLogPvalBin'){
    name = 'Binomial Fit Score'
  }else if(var == 'minusLogPermU'){
    name = 'Permuted U Test Fit Score'
  }else if(var == 'minusLogPermBin'){
    name = 'Permuted Binomial Test Fit Score'
  }else if(var == 'finalScore'){
    name = 'Combined Score (S)'
  }else if(var == 'match_rate'){
    name = 'Match Rate'}

  ggplot(data = box.mat,aes(y = !!sym(var)))+
    geom_boxplot(data = filter(box.mat,group!='MPH'), colour = 'black',fatten = 3,lwd = 0.5,width = 0.25,
                 aes(x = reorder(group,group),fill = group),outliers = FALSE)+
    geom_jitter(data = filter(box.mat,group!='MPH'), colour = 'black',width = 0,alpha = 0.25,
                aes(x = reorder(group,group),fill = group))+
    geom_point(data = filter(box.mat,group=='MPH'), color = 'red',
               aes(x = group))+
    
    geom_text_repel(data = filter(box.mat,group=='MPH'),max.overlaps = Inf,
                    nudge_x = 0.25, nudge_y = 0,
                    aes(label = specimen, x = group),direction = "y",seed = 2024,
    ...
    theme(legend.position='none',
          ...
          panel.grid.minor = element_blank())+
    labs(y = name, x = '')+
    scale_x_discrete(limits = c('ERC','AMH','NE','MPH'),
                     labels = c('ERC' = 'H.erectus' ,'AMH' = 'AMH',
                                'NE' = 'Neanderthals','MPH' = 'Test Subjects'))+
    #coord_flip()+
    scale_fill_manual(values = c('MPH' = 'black', 'NE' = '#FBB040', 'AMH' = '#3B94D1', 'ERC' = 'palegreen3'))
  
  #ggsave(filename = paste('results/confetti_plot/',var,'.svg',sep = ''), width = 12,height = 7)
...
p = ggplot(box.mat,aes(color = group,x = -log10(binom.p.val), y = -log10(wilcox.p.val)))+
  geom_point(data = filter(box.mat,group != 'MPH'),aes(color = group),size = 5,alpha = 0.5,shape = 16)+
  geom_point(data = filter(box.mat,group == 'MPH'),aes(color = group),size = 8,alpha = 0.4,shape = 16)+
  geom_text_repel(data = filter(box.mat,group %in% c('MPH')),
                  nudge_y = 0.25, size = 14*5/14,
                  aes(label = specimen), max.overlaps = Inf, color = 'red')+
  ...
  theme(panel.background = element_blank())+
    ...
    legend.frame = element_rect(colour = "black", fill=NA, size=0.5),
  ...
  p = ggMarginal(p, type="boxplot",groupFill = TRUE, fatten = 3,lwd = 0.5,alpha = 0.5,)


  #ggsave(p, filename = paste('results/confetti_plot/pval_dotsAllNames.svg',sep = ''), width = 9,height = 9)

```
Confetti plot for archaic comparison
...
p = ggplot(box.mat,aes(color = group,x = -log10(binom.p.val), y = -log10(wilcox.p.val)))+
  ...
  #ggsave(p, filename = paste('final graphs after revision/archaic_comparison/confetti_archaic_comparison.svg',sep = ''), width = 9,height = 9)
...
ggplot(data = pvals,aes(x = group, y = wilcox.p.val))+
  geom_violin()+
  geom_label(aes(label = specimen))+
  scale_y_continuous(trans = "log10")

ggplot(data = pvals,aes(x = group, y = perm_success_U))+
  geom_violin()+
  geom_label_repel(aes(label = specimen))

m = full_craniometric_data
m$group <- factor(m$group, levels = c(levels(m$group), 'AMH'))

m[m$group %in% c('EHS','UPS'),'group'] = 'AMH'

slice(m,-c(1,2))%>%
  filter(group %in% c('AMH','NE'))%>%
  ggplot(aes(x = group,y = glab.prot.neu.nm))+
  geom_boxplot()+
  geom_label_repel(aes(label = specimen))
...
AMH = filter(m,group=='AMH')%>%
  select(glab.prot.neu.nm)

NE = filter(m,group=='NE')%>%
  select(glab.prot.neu.nm)

UPS = filter(m,group=='UPS')%>%
  select(glab.prot.neu.nm)


t.test(AMH,NE,alternative = 'g')
t.test(UPS,NE,alternative = 'g')


full_craniometric_data$glab.prot

ggplot(data = m,aes(x = group, y = glab.prot.neu.nm))+
  geom_boxplot()+
  geom_label_repel(aes(label = specimen))
...
#Den Vs Ne predictions

```{r}
#First run the algorithm with only Denisovan-specific predictions, save as Denisovan_specific_score
#Second, run again with only Neanderthal-specific predictions, save as Neanderthal_specific_score

combined_nea_den <- merge(Denisovan_specific_score, select(Neanderthal_specific_score,c(specimen,wilcox.score)), by = "specimen", all = TRUE,suffixes = c(".denis",".nean"))

combined_nea_den$group = as.vector(combined_nea_den$group)
combined_nea_den[combined_nea_den$group %in% c('MPH'),'group'] = 'MPH'
combined_nea_den$group = as.factor(combined_nea_den$group)
combined_nea_den$group = factor(combined_nea_den$group, levels=c("MPH", "NE"))
combined_nea_den$specimen = fix.names(combined_nea_den$specimen)

combined_nea_den$den_min_nea = combined_nea_den$wilcox.score.denis-combined_nea_den$wilcox.score.nean

p = ggplot(data = combined_nea_den,aes(x = wilcox.score.denis, y =wilcox.score.nean,color = group))+
  geom_point(data = filter(combined_nea_den,group != 'MPH'),aes(color = group),size = 5,alpha = 0.5,shape = 16)+
  geom_point(data = filter(combined_nea_den,group == 'MPH'),aes(color = group),size = 5,alpha = 0.5,shape = 16)+
  geom_text_repel( size = 14*5/14,
                  aes(label = specimen), max.overlaps = Inf)+
  scale_color_manual(values = c('MPH' = 'red', 'NE' = '#FBB040', 'AMH' = '#3B94D1',
                                'ERC' = 'palegreen3'),
                     ...
                     labels = c('MPH' = 'Test subjects', 'NE' = 'Neanderthals',
                                'AMH' = 'Anatomically modern humans', 'ERC' = 'H. erectus'))+
  geom_abline(slope = 0.5, intercept = 0, color = "lightgray",linewidth = 1) +
  theme_void()+
  
  labs(y = 'Neanderthal-specific U-test score', x = 'Denisovan-specific U-test score')+
  theme(panel.background = element_blank())+
    theme(
    panel.background = element_blank(),
    axis.title = element_text(size = 18),  # Adjust the size of axis titles
...
ggsave(p, filename = paste('results/confetti_plot/Den_vs_Nea_plot.svg',sep = ''), width = 10,height = 7)
...
# Neanderthals vs Denisovan score analysis DEPRACTED

```{r}
combined_neanderthal <- merge(pvals, select(neanderthals,c(specimen,comb_score)), by = "specimen", all = TRUE)
combined_neanderthal <- merge(pvals, select(neanderthals,c(specimen,wilcox.score)), by = "specimen", all = TRUE)


combined_neanderthal$group = as.vector(combined_neanderthal$group)
combined_neanderthal[combined_neanderthal$group %in% c('EHS','UPS'),'group'] = 'AMH'
combined_neanderthal[combined_neanderthal$group %in% c('ERC_EP','ERC_MP'),'group'] = 'ERC'
combined_neanderthal[combined_neanderthal$group %in% c('MPH'),'group'] = 'MPH'
combined_neanderthal$group = as.factor(combined_neanderthal$group)
combined_neanderthal$group = factor(combined_neanderthal$group, levels=c("MPH", "NE","ERC","AMH"))
combined_neanderthal$specimen = fix.names(combined_neanderthal$specimen)
combined_neanderthal = combined_neanderthal %>%
  filter(total_predictions>4)




ggplot(data = combined_neanderthal,aes(x = wilcox.score.x, y = wilcox.score.y,color = group))+
  geom_point(data = filter(combined_neanderthal,group != 'MPH'),aes(color = group),size = 5,alpha = 0.5,shape = 16)+
  geom_point(data = filter(combined_neanderthal,group == 'MPH'),aes(color = group),size = 8,alpha = 0.4,shape = 16)+
  geom_text_repel(data = filter(combined_neanderthal,group %in% c('MPH')),
                  nudge_y = 0.25, size = 14*5/14,
                  aes(label = specimen), max.overlaps = Inf, color = 'red')+
  scale_color_manual(values = c('MPH' = 'red', 'NE' = '#FBB040', 'AMH' = '#3B94D1',
                                'ERC' = 'palegreen3'),
